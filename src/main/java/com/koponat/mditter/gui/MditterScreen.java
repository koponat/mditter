...
public class MditterScreen extends Screen {
    ...
    public MditterScreen() { super(Text.literal("MDITTER v1.5")); }
    @Override
    protected void init() {
        int x = this.width / 2 - 100;
        int y = this.height / 2 - 60;
        this.addDrawableChild(ButtonWidget.builder(getLangText("数据包拦截: 关闭", "Packet Blocker: OFF"), b -> {}).dimensions(x, y, 200, 20).build());
        this.addDrawableChild(ButtonWidget.builder(getLangText("抓包器: 未就绪", "Packet Sniffer: Not Ready"), b -> {}).dimensions(x, y + 25, 200, 20).build());
    ...
    private Text getLangText(String zh, String en) { return Text.literal(isEnglish ? en : zh); }
    @Override
